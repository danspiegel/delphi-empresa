...
begin
  If rbTipoSmtp.Checked
    Then begin
      If (edtServSMTP.Text = '')
        Then MessageBox(Handle, PChar('O campo Servidor é obrigatório.'), 'Atenção', MB_ICONEXCLAMATION + MB_OK)
      Else If (edtPortaServSMTP.Text = '')
        Then MessageBox(Handle, PChar('O campo Porta é obrigatório.'), 'Atenção', MB_ICONEXCLAMATION + MB_OK)
      Else If (edtUsuarioServSMTP.Text = '')
        Then MessageBox(Handle, PChar('O campo usuário é obrigatório.'), 'Atenção', MB_ICONEXCLAMATION + MB_OK)
      Else If (edtSenhaServSMTP.Text = '')
        Then MessageBox(Handle, PChar('O campo senha é obrigatório.'), 'Atenção', MB_ICONEXCLAMATION + MB_OK)
      Else begin

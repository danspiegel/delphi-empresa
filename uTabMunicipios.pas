...
begin
  If (edtLocalizarTabMunicipios.Text = '')
    Then MessageBox(handle, pChar('O texto para busca deve ser informado.'), 'Atenção', MB_ICONEXCLAMATION + MB_OK)
  else begin
    with QLocMunic
      Do begin
        Close;
...
begin
  ...
  MessageDlg('A tabela já está atualizada.', mtInformation, [mbOK], 0);
end;
